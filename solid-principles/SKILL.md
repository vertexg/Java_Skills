...
description: Java 例付きの SOLID 原則チェックリストです。クラス設計のレビュー、リファクタリング、Single Responsibility / Open-Closed / Liskov / Interface Segregation / Dependency Inversion の相談時に使います。
---

# SOLID Principles スキル

Java コードに SOLID 原則を適用し、設計を見直すためのスキルです。
...
## 使うタイミング
- ユーザーが「SOLID を見て」「SOLID 観点でレビューして」「このクラスは責務が多すぎる？」と依頼したとき
- クラス設計をレビューするとき
- 大きなクラスをリファクタリングするとき
- 設計重視のコードレビューをするとき
