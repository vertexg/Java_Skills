...
description: null 安全性、例外処理、並行処理、性能観点を含む Java 向けの体系的なコードレビュー用スキルです。「コードレビューして」「この PR を見て」「レビューして」「PR を見て」「コード RV」などの依頼や、マージ前の確認で使います。
---

# Java Code Review スキル

Java プロジェクト向けの体系的なコードレビューチェックリストです。
...
## 使うタイミング
- ユーザーが「このコードをレビューして」「この PR を見て」「コードレビューして」と依頼したとき
- PR をマージする前
- 機能実装の直後
...
レビュー対象に次の要素が含まれる場合は、`java-code-review` だけで閉じず、関連スキルの観点も使って確認する。

- **SQL 文字列連結、入力値の直埋め、認証・認可、秘密情報** → `security-audit`
- **ループ内の重い処理、不要なオブジェクト生成、文字列連結、性能懸念** → `performance-smell-detection`
- **JPA / Hibernate / Repository / N+1 / LazyInitializationException** → `jpa-patterns`
- **責務過多、肥大化したクラス、依存関係の粗さ** → `solid-principles`
- **命名、重複、長いメソッド、可読性の低さ** → `clean-code`

レビュー依頼が広い場合でも、少なくとも **セキュリティ・リソース管理・可読性・性能** の観点を意識して横断的に確認する。
